...
if [ "$2" = "issuer" ]; then
      helm upgrade sda /Users/astralmiau/work/sda-helm-viktoriaas/charts/sda-svc --install -f /Users/astralmiau/work/sda-helm-viktoriaas/dev_tools/scripts/sda-svc.yaml \
          --set global.archive.s3AccessKey="$S3_ACCESS_KEY",global.archive.s3SecretKey="$S3_SECRET_KEY",global.backupArchive.s3AccessKey="$S3_ACCESS_KEY",global.backupArchive.s3SecretKey="$S3_SECRET_KEY",global.c4gh.passphrase="$C4GH_PASSPHRASE",global.db.passIngest="$DB_IN_PASS",global.db.passOutgest="$DB_OUT_PASS",global.inbox.s3AccessKey="$S3_ACCESS_KEY",global.inbox.s3SecretKey="$S3_SECRET_KEY",global.schemaType="$SCHEMA",intercept.deploy="$INTERCEPT",releasetest.secrets.accessToken="$token" -n ega-ns
